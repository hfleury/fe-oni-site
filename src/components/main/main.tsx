...
type BlockType = {
    type: string;
    elements: { [key: string]: ComponentElement };
};
...
type MainProps = {
    ...
    buttonRoundedCss?: string;
    landingPages?: { [key: string]: ComponentElement };
    innerPages?: { [key: string]: ComponentElement };
    blocks?: { [key: string]: BlockType };
    heading?: string;
    description?: string;
};
...
const Main: React.FC<MainProps> = ({
    ...
    useEffect(() => {
        ...
        (window as any).gtag("config", "UA-45799926-9");
    }, []);

    const previewImageAnimationVariants: Variants = {
        rest: {
    ...
    const noOfComponentBlocks = Object.values(blocks).reduce(
        (acc: number, block: BlockType) => acc + Object.keys(block.elements).length,
        0
    );
...
const Block: React.FC<BlockProps> = ({ notifyIsVisible, components }) => {
    ...
    // Use a ref to persist iframe refs across renders
    const componentBlockRefs = useRef<{ [url: string]: HTMLIFrameElement | null }>({});
    ...
    return (
        <div ref={ref as React.RefObject<HTMLDivElement>} tw="mt-32">
                ...
                {Object.values(components.elements).map((component, componentIndex) => (
                    <Component key={componentIndex}>
                        <ComponentHeading>
                            ...
                            <ComponentPreviewLink className="group" href={component.url} target="_blank" rel="noopener noreferrer">
                                View Live Demo{" "}
                        ...
                        <ComponentContent>
                            <ResizableBox
                                ...
                                onResize={() => updateComponentBlockIframeHeight(componentBlockRefs.current[component.url])}
                            >
                                <iframe
                                    src={component.url}
                                    title={component.name}
                                    width="100%"
                                    ref={ref => (componentBlockRefs.current[component.url] = ref)}
                                    onLoad={e => updateComponentBlockIframeHeight(e.currentTarget)}
